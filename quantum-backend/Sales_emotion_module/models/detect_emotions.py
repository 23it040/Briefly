...
import torch
import numpy as np
from .emotion_model import load_model, predict_emotion, KerasEmotionModel
...
class SalesCallAnalyzer:
    ...
    def __init__(self, model_path=MODEL_PATH, model_type_prod='keras', window_size=30):
        ...
        # Load model using the new centralized loading function
        self.model, self.metadata = load_model(model_path, model_type_prod)
        ...
        if not isinstance(self.model, KerasEmotionModel):
            ...
            self.device = None  # Keras models handle device internally
        ...
        self.model_type_prod = model_type_prod
        if not isinstance(self.model, KerasEmotionModel):
            self.transform = self.get_preprocessing_pipeline(model_type_prod)
        else:
            self.transform = None  # Keras models handle preprocessing internally
    ...
    def get_preprocessing_pipeline(self, model_type_prod):
        ...
        Args:
            model_type_prod (str): Type of model to use
    ...
    def preprocess_face(self, face_img):
        ...
        Returns:
            torch.Tensor or numpy.ndarray: Preprocessed input for the model
        """
        if isinstance(self.model, KerasEmotionModel):
            # For Keras models, return the numpy array directly
            # The KerasEmotionModel handles preprocessing internally
            return face_img
        else:
            # For PyTorch models, apply transforms
    ...
    def detect_emotion(self, frame):
        ...
        for (x, y, w, h) in faces:
            ...
            try:
                ...
                # Make prediction using the unified predict_emotion function
                if isinstance(self.model, KerasEmotionModel):
                    # For Keras models
                    ...
                    # Map emotion index to label for PyTorch models
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                try:
                    ...
                    # Preprocess face
                    face_input = analyzer.preprocess_face(face_roi)
                    
                    # Make prediction
                    if isinstance(analyzer.model, KerasEmotionModel):
                        # For Keras models
                        prediction = predict_emotion(analyzer.model, face_input)
                        emotion = prediction['emotion']
                        confidence = prediction['confidence']
                    else:
                        # For PyTorch models
                        face_input = face_input.to(analyzer.device)
                        prediction = predict_emotion(analyzer.model, face_input)
