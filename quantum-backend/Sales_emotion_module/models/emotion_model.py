...
# TensorFlow/Keras imports for external model support
try:
    ...
    print("TensorFlow/Keras not available. External .h5 models will not be supported.")
...
class KerasEmotionModel:
    ...
    def __init__(self, model_path):
        if not TENSORFLOW_AVAILABLE:
            raise ImportError("TensorFlow/Keras is required for loading .h5 models")
    ...
    def predict(self, face_image):
        ...
        try:
            ...
            # Add channel dimension and batch dimension: (1, 48, 48, 1)
            ...
            return {
                ...
                'emotion_idx': -1,
                'confidence': 0.0,
                'probabilities': [0.0] * len(self.emotion_labels),
                'is_reliable': False
            }
...
def load_model(model_path, model_type='modern'):
    ...
    if model_path.endswith('.h5') or model_type == 'keras':
        if not TENSORFLOW_AVAILABLE:
            raise ImportError("TensorFlow/Keras is required for loading .h5 models")
        
        model = KerasEmotionModel(model_path)
        metadata = {
            'type': 'keras',
            'model_format': 'h5',
    ...
    logger.info(f"Loading {model_type} PyTorch model from {model_path}")

    # Initialize the appropriate PyTorch model
    ...
    try:
        ...
        logger.info(f"Successfully loaded {model_type} PyTorch model")
...
def predict_emotion(model, face_tensor_or_image, confidence_threshold=0.5):
    """Predict emotion from face tensor/image with confidence threshold.
    
    Works with both PyTorch and Keras models.
    
    Args:
        model: PyTorch model or KerasEmotionModel instance
        face_tensor_or_image: For PyTorch models: preprocessed tensor, For Keras models: numpy array
        confidence_threshold: Minimum confidence for reliable predictions
        
    Returns:
        Dictionary with prediction results
    """
    if isinstance(model, KerasEmotionModel):
        # Use Keras model prediction
        return model.predict(face_tensor_or_image)
    else:
        # Use PyTorch model prediction
        with torch.no_grad():
            outputs = model(face_tensor_or_image)
